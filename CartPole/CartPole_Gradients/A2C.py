...
class Reinforce_actor(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        # Optional: use an extra hidden layer for more capacity
        # self.fc2 = nn.Linear(128, 64)
        self.policy = nn.Linear(128, output_dim)

    def forward(self, x):
        x = torch.relu(self.fc1(x))
        # x = torch.relu(self.fc2(x))  # Uncomment if using second hidden layer
...
def main(args):
    ...
    learning_rate = 0.001
    num_runs = 5
    n = args.n             # depth parameter as input  
    max_steps = 10**6    
    normalize_advantage = args.norm   # boolean nromalization as input
    ...
    # Training Loop
    global_records = []  # List to store (global_steps, episode_reward) per run
    ...
    for run in range(num_runs):
        ...
        while global_steps < max_steps:
            ...
            # Run until global step limit is reached
            while not done:
                global_steps += 1
            ...
            for t in range(len(trace)):
                ...
                actor_loss += -log_prob_t * advantages_normalized[t]
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Advantage Actor-Critic with TD bootstrapping")
  parser.add_argument(
    "--n",
    type=int,
    default = 50,
    help="Number of steps for n-step TD bootstrapping (default: 50), we have tried 20 and 30 as well"
  )
  parser.add_argument(
    "--norm",
    type=bool,
    default = True,
    help="Normalization of advantage (default: True)"
  )
